/**
 * TGDĐ LAPTOP SCRAPER — Final v2
 * Fix: Click "Xem thêm" liên tục để load toàn bộ sản phẩm
 */
...
async function scrapeBrand(page, brand) {
  ...
  try {
    ...
    console.log(`  ⚠ Lỗi load trang: ${e.message.substring(0,60)}`);
    return [];
  }

  await sleep(2000);

  // ── Click "Xem thêm" liên tục cho đến hết ──────────────
  let clickCount = 0;
  while (true) {
    await autoScroll(page);
    await sleep(1500);

    // Tìm nút "Xem thêm" — TGDĐ dùng nhiều dạng khác nhau
    const clicked = await page.evaluate(() => {
      // Thử nhiều selector
      const selectors = [
        '.view-more a:not(.prevent)',
        'a.btn-more',
        'a.view-more',
        '[class*="view-more"] a',
        'a[class*="loadmore"]',
    ...
    if (!clicked) {
      console.log(`  → Đã load hết (click ${clickCount} lần)`);
      break;
    }

    clickCount++;
    console.log(`  → Click "Xem thêm" lần ${clickCount}...`);
    await sleep(2500); // Đợi load thêm sản phẩm
  }

  // ── Parse toàn bộ sản phẩm sau khi load hết ──────────
  const products = await page.evaluate((brandName, BASE) => {
    ...
    document.querySelectorAll('ul.listproduct li.item').forEach(item => {
      ...
      if (!fullName || !href || seenLinks.has(link)) return;
      seenLinks.add(link);
