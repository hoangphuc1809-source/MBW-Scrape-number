/**
 * multi_dealer_scraper.js  — v2.4
 *
 * FIX v2.4:
 *  [BUG6] CPS không có specs vì FPT dùng hết quota chung
 *         → Tách counter riêng: FPT_MAX = 30, CPS_MAX = 30 (độc lập nhau)
 */
...
const FPT_MAX_SPEC_FETCH = 30; // [BUG6] Tối đa 30 specs/lần cho FPT (độc lập CPS)
const CPS_MAX_SPEC_FETCH = 30; // [BUG6] Tối đa 30 specs/lần cho CPS (độc lập FPT)
...
// ── Fetch specs với giới hạn riêng cho từng dealer ────────
// [BUG6] maxFetch truyền vào độc lập → FPT và CPS mỗi cái có 30 slots riêng
async function enrichSpecs(products, specCache, fetchFn, page, specFetchCount, maxFetch) {
  for (const p of products) {
    if (specCache.has(p.link)) {
      Object.assign(p, specCache.get(p.link));
    } else if (specFetchCount.value < maxFetch) {
      console.log(`    → Specs [${specFetchCount.value+1}/${maxFetch}]: ${p.name.substring(0,40)}`);
...
async function scrapeMBW(page, brand) {
  ...
  return page.evaluate((brandName, BASE) => {
    ...
    document.querySelectorAll('ul.listproduct li.item').forEach(item => {
      ...
      // MBW thứ tự specs: [0]=Màn hình, [1]=CPU, [2]=GPU, [3]=Trọng lượng
      // Dùng text prefix để tránh phụ thuộc thứ tự cố định
      const findSpec = (keyword) => specs.find(s => s.toLowerCase().includes(keyword)) || '';
      const screen  = findSpec('màn hình') || findSpec('inch') || specs[0] || '';
      const cpu     = findSpec('cpu') || findSpec('core') || findSpec('ryzen') || findSpec('celeron') || findSpec('snapdragon') || findSpec('mediatek') || specs[1] || '';
      const gpu     = findSpec('card') || findSpec('rtx') || findSpec('gtx') || findSpec('radeon') || findSpec('arc') || findSpec('intel graphics') || findSpec('uma') || specs[2] || '';
      const weight  = findSpec('kg') || findSpec('trọng lượng') || findSpec('hàng') || specs[3] || '';
...
async function scrapeFPT(page, brand, specCache, specFetchCount) {
  ...
  await enrichSpecs(products, specCache, fetchSpecsFPT, page, specFetchCount, FPT_MAX_SPEC_FETCH);
  return products;
}
...
async function scrapeCPS(page, brand, specCache, specFetchCount) {
  ...
  console.log(`    → ${products.length} SP`);
  await enrichSpecs(products, specCache, fetchSpecsCPS, page, specFetchCount, CPS_MAX_SPEC_FETCH);
  return products;
}
...
(async () => {
  ...
  const allProducts  = [];
  // [BUG6] Counter riêng cho FPT và CPS — độc lập nhau
  const fptSpecCount = { value: 0 };
  const cpsSpecCount = { value: 0 };
  ...
  try {
    ...
    for (const brand of BRANDS) {
      const products = await scrapeFPT(pageFPT, brand, specCache, fptSpecCount);
      allProducts.push(...products);
      await sleep(800);
    }
    await pageFPT.close();
    ...
    for (const brand of BRANDS) {
      const products = await scrapeCPS(pageCPS, brand, specCache, cpsSpecCount);
      allProducts.push(...products);
      await sleep(800);
    }
    await pageCPS.close();
  ...
  console.log(`   Specs fetched: FPT ${fptSpecCount.value}/${FPT_MAX_SPEC_FETCH} | CPS ${cpsSpecCount.value}/${CPS_MAX_SPEC_FETCH}`);
