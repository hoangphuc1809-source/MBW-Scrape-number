...
const FPT_MAPPING_VERSION = 2; // v2.8: fix GPU dual card + RAM/Storage/Screen fields
...
async function scrapeMBW(page) {
  ...
  while (true) {
    ...
    const result = await page.evaluate(() => {
      // Thử tất cả selector của button "Xem thêm" trên MBW
      const selectors = [
        'a.view-more', 'button.view-more', '.view-more-btn',
        'a[class*="view-more"]', 'button[class*="view-more"]',
        '.btn-view-more', 'a.btn-more', '.show-more',
      ];
  ...
  return page.evaluate((BASE) => {
    ...
    document.querySelectorAll('ul.listproduct li.item').forEach(item => {
      ...
      // data-price trên MBW đơn vị là 1000đ → nhân 10 để ra đồng
      const salePrice = (parseInt((aEl.getAttribute('data-price') || '0').replace(/\D/g,'')) || 0) * 10;
...
(async () => {
  ...
  console.log('🚀 Multi-Dealer Scraper v2.8');
