...
async function scrapeMBW(page, brand) {
  ...
  return page.evaluate((brandName, BASE) => {
    ...
    document.querySelectorAll('ul.listproduct li.item').forEach(item => {
      ...
      // [BUG8 FIX] Dùng keyword search thay vì index cứng
      // MBW thứ tự thực tế: [0]=Màn hình, [1]=CPU, [2]=GPU, [3]=Trọng lượng
      const findSpec = (kw) => specs.find(s => s.toLowerCase().includes(kw)) || '';
      ...
      const gpu     = findSpec('card') || findSpec('rtx') || findSpec('gtx') || findSpec('radeon') || findSpec('arc') || findSpec('intel graphics') || specs[2] || '';
      const weight  = findSpec('kg') || findSpec('trọng lượng') || specs[3] || '';
...
(async () => {
  ...
  console.log('🚀 Multi-Dealer Scraper v2.5');
